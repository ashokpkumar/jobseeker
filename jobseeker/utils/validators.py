...
def loginrequired(func): # validates actions only when login time is within expired time
    @wraps(func)
    def inner1(request, *args, **kwargs):
        bearer_token = request.headers.get("authorization").split(" ")[1]
        # required_token = bearer_token.split(" ")
        # only_token = required_token[1]
        return_payload = jwt_decode(bearer_token) # return_payload is in dict format after decoding
        i_exp = return_payload["exp_at"] # expired date time is in string format
        user_email = return_payload["user_email"] # getting user_email from return_payload dict
...
def validate_body(func): # validated the data in the form fields
    @wraps(func)
    def inner4(request, *args, **kwargs):
        print("before request")
        validator_class = rbm_map[request.resolver_match.view_name]
